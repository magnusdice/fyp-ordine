  ...
  <div class="flex flex-row h-screen">
    ...
    <main class="w-full overflow-x-hidden">
      <div class="flex justify-between py-3 px-6 sticky top-0 z-10 bg-neutral border-b-2">
        ...
        <div>
          <a href="emp-add-page.php" class="btn btn-primary">Add Staff</a>
        </div>
      </div>
        ...
        <table class="table">
          ...
          <tbody>
            ...
            if ($restaurant_id === null) {
              ...
              $emp_search_sql = isset($_GET['search'])
              ? "SELECT e.employee_id, e.firstName, e.lastName, e.empDate, e.empSalary, e.empRoles, e.empImage, e.empNo,e.restaurant_id, e.user_id,u.email
              FROM employee e
              LEFT JOIN user u ON e.user_id = u.user_id
              WHERE e.restaurant_id = $restaurant_id AND CONCAT(e.firstName, e.lastName,e.empRoles)
              LIKE '%$filtervalues%'" 
              : "SELECT e.employee_id, e.firstName, e.lastName, e.empDate, e.empSalary, e.empRoles, e.empImage, e.empNo,e.restaurant_id, e.user_id,u.email
              FROM employee e
              LEFT JOIN user u ON e.user_id = u.user_id
              ...
              $emp_search_sql_result = mysqli_query($mysqli, $emp_search_sql);
              if (mysqli_num_rows($emp_search_sql_result) > 0) {
                  foreach ($emp_search_sql_result as $row) {
            ...
            ?>
                    <tr class="hover">
                      ...
                      <th>
                        <div>
                          <a class="btn btn-xs my-1 btn-primary" href="emp-edit-page.php?employee_id=<?php echo $row["employee_id"]; ?>">Edit</a>
                        </div>

                        <form action="CRUD-employee-functions.php" method="POST">
                          <input type="hidden" name="employee_id" value="<?php echo $row["employee_id"]; ?>">
                          <input type="hidden" name="user_id" value="<?php echo $row["user_id"]; ?>">
                          <input type="hidden" name="empImage" value="<?php echo $row["empImage"]; ?>">
                          <button type="submit" name="delete-emp-data" class="btn btn-xs my-1 btn-error text-white">Delete</button>
                        </form>
